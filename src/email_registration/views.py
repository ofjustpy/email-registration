...
def on_loginbtn_click(dbref, msg, form_inputs):
    ...
    var username = document.getElementById('/inputbox_email').value;
    console.log("username");
    var password= document.getElementById('/inputbox_password').value;
    console.log("password");
    console.log(username);
    console.log(password)
    xhr.open("GET", "{login_page_url}", true);
...
def wp_user_login(request):
    ...
    with oj.sessionctx(session_manager):
        ...
        testbtn_ = oj.Button_("testbtn",
                             text="test",
                             type="submit").event_handle(oj.click, on_loginbtn_click)
        

        wp_ = oj.WebPage_("loginpage",
                          cgens =[target_, testbtn_],
                          template_file='svelte.html',
